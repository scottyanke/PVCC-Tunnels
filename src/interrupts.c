 ...
 * MA 02110-1301, USA.
 */
// the uart receive and timer 4 interrupts are coded here
...
INTERRUPT_HANDLER(UART1_RX_IRQHandler, 18){
	uint8_t rb;
	if(UART1->SR & UART1_SR_RXNE){ // data received
		rb = UART1->DR; // read received byte & clear RXNE flag
...
void TIM4_UPD_OVF_IRQHandler() __interrupt(23){
	if(TIM4->SR1 & TIM4_SR1_UIF){ // update interrupt
		Global_time++; // increase timer 1 tick every ms
	}
